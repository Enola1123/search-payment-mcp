...
| ClawHub | 脚本 `search_registries.py` | ✅ | 否 | `GET /api/v1/packages/search?q=`，返回 isOfficial 信号 |
| Smithery | 脚本 `search_registries.py` | ✅ | 否 | `GET /api.smithery.ai/servers?q=`，100k+ 条目，全文本+语义搜索 |
| Glama | 脚本 `search_registries.py` | ✅ | 否 | `GET /api/mcp/v1/servers?query=`，65k+ 条目，全文搜索 |
| PulseMCP | 脚本 `search_registries.py` | ✅ | 需 Key | `GET /api.pulsemcp.com/v0.1/servers?search=`，需 X-API-Key + X-Tenant-ID |
...
- **重要**: 必须使用 `/api/v1/packages/search?q=` 端点。**不能**使用 `/api/v1/packages?q=`——后者不接受 `q` 参数，会忽略搜索词并返回默认列表。
- **认证**: 搜索**无需 Token**，公开可用
- **返回格式**: `{"results": [{"score": N, "package": {...}}, ...]}`，每个 `package` 含 `name`、`displayName`、`summary`、`family`（skill/plugin）、`ownerHandle`、`isOfficial`、`stats.downloads`
...
### Smithery Registry — API 检索

- **API**: `GET https://api.smithery.ai/servers?q=<keyword>&pageSize=<N>`
- **认证**: 基础搜索**无需 Token**，公开可用。完整 API 需 Bearer Token（在 https://smithery.ai/account/api-keys 创建）
- **返回格式**: `{"servers": [...], "pagination": {"currentPage", "pageSize", "totalPages", "totalCount"}}`
- **搜索策略**: 全文本 + 语义搜索，100k+ 条目。支持高级过滤语法：`owner:<username>`、`repo:<name>`、`is:deployed`、`is:verified`
- **归属验证信号**:
  - `qualifiedName`（格式 `namespace/name`）的 namespace 是否为公司官方组织名
  - `verified` 是否为 TRUE（由 Smithery 验证）
  - `homepage` 是否指向公司官方域
  - `bySmithery` 是否为 TRUE（表示由 Smithery 官方构建和发布）

### Glama MCP Directory — API 检索

- **API**: `GET https://glama.ai/api/mcp/v1/servers?query=<keyword>&first=<N>`
- **认证**: 搜索**无需 Token**，公开可用
- **参数**: `query` 全文搜索（必填），`first` 返回数量 1-100（默认 10），`after` 游标分页
- **搜索策略**: 全文搜索，65k+ 条目。直接返回命中结果，无需本地过滤。
- **返回格式**: `{"pageInfo": {"endCursor", "hasNextPage", ...}, "servers": [...]}`
- **归属验证信号**:
  - `namespace` 是否为公司官方组织名
  - `name` 是否包含公司英文名/品牌名
  - `repository.url` 是否指向公司官方 GitHub 仓库

### PulseMCP Registry — API 检索

- **API**: `GET https://api.pulsemcp.com/v0.1/servers?search=<keyword>&limit=<N>&version=latest`
- **认证**: **需要** API Key + Tenant ID。发送邮件至 `hello@pulsemcp.com` 申请
- **配置方式**: 将凭证写入 skill 目录下的持久化文件：
  ```bash
  ...
  ```
- **返回格式**: `{"servers": [{"server": {...}, "_meta": {...}}], "metadata": {"nextCursor", "count"}}`
- **搜索策略**: 按服务器名称/标题子串匹配；`version=latest` 仅返回最新版本
- **速率限制**: 200次/分钟，5,000次/小时，10,000次/天
- **归属验证信号**:
  - `_meta.com.pulsemcp/server.isOfficial` — 最可靠的官方信号
  - `server.name` 是否包含公司英文名
  - `server.websiteUrl` 是否指向公司官方域
  - `server.repository.url` 是否指向公司官方 GitHub
  - `visitorsEstimate*` 访问量估算（热门程度参考）
...
- 通联支付: prodoc.allinpay.com/doc/2714（AI Skills 使用指引，链接 Skillhub 下载地址）
- 发现新的公司自有入口后，更新本节列表
